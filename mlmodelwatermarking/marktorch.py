...
from mlmodelwatermarking.loggers.logger import (
    logger,
)
from mlmodelwatermarking.verification import (
    verify,
)
...
class Trainer:
    def __init__(
        self, model, args, trainset=None, valset=None, testset=None, specialset=None
    ):
    ...
    def loaders(self, trainset=None):
        ...
        valloader = DataLoader(
            self.valset, batch_size=self.args.batch_size, shuffle=True
        )
    ...
    def generate_trigger_merrer(self):
        ...
        for data, target in trainloader:
            ...
            if final_pred.item() != target.item():
                ...
                triggerset.append(
                    (perturbed_data.reshape(shape[1:]), final_pred.item())
                )
    ...
    def __is_prediction_dawn(self, item):
        ...
        """
        bound = floor((2**self.args.precision_dawn) * self.args.probability_dawn)
        hashed = hmac.new(
            self.args.key_dawn.encode("utf-8"), item.tobytes(), hashlib.sha256
        ).hexdigest()
